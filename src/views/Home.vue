<template>
  <div class="home">
    <HomeHeader :gender="gender" />
    <van-swipe
      class="my-swipe"
      :autoplay="3000"
      indicator-color="white"
      :show-indicators="false"
    >
      <van-swipe-item
        v-for="(item, index) in gender === 'male' ? bannerMale : bannerFemale"
        :key="index"
      >
        <van-image width="100%" :src="item.img" />
      </van-swipe-item>
    </van-swipe>

    <van-tabbar :fixed="false" :border="false" @tabbar-height="38.4">
      <van-tabbar-item
        icon="data:image/png;base64,"
        >书架</van-tabbar-item
      >
      <van-tabbar-item
        icon="data:image/png;base64,"
        @click="$router.push('/ranking')">排行</van-tabbar-item
      >
      <van-tabbar-item
        icon="data:image/png;base64,"
        >VIP</van-tabbar-item
      >
      <van-tabbar-item
        icon="data:image/png;base64,"
        >充值</van-tabbar-item
      >
    </van-tabbar>

    <div class="zuijin">
      <span>最近阅读</span>
      <span>|</span>
      <span>暂无阅读记录</span>
    </div>

    <HomeList v-if="maleHot && gender === 'male'" :booklists="maleHot" :id="'54d42d92321052167dfb75e3'" :title="'男生热门'">男生热门</HomeList>
    <HomeList v-if="maleFinish && gender === 'male'" :booklists="maleFinish" :id="'564eb878efe5b8e745508fde'" :title="'男生完本'">男生完本</HomeList>
    <HomeList v-if="maleLiuchunlv && gender === 'male'" :booklists="maleLiuchunlv" :id="'564547c694f1c6a144ec979b'" :title="'男生大神区'">男生大神区</HomeList>

    <HomeList v-if="femaleHot && gender === 'female'" :booklists="femaleHot" :id="'54d43437d47d13ff21cad58b'" :title="'畅销短篇'">畅销短篇</HomeList>
    <HomeList v-if="femaleFinish && gender === 'female'" :booklists="femaleFinish" :id="'564eb8a9cf77e9b25056162d'" :title="'女生佳作'">女生佳作</HomeList>
    <HomeList v-if="femaleLiuchunlv && gender === 'female'" :booklists="femaleLiuchunlv" :id="'5645482405b052fe70aeb1b5'" :title="'女生红文区'">女生红文区</HomeList>

    <BottomZhu />
  </div>
</template>

<script>
// @ is an alias to /src
import HomeHeader from "@/components/HomeHeader.vue";
import HomeList from "@/components/HomeList.vue";
import BottomZhu from "@/components/BottomZhu.vue";

export default {
  name: "Home",
  components: {
    HomeHeader,
    HomeList,
    BottomZhu
  },
  data() {
    return {
      gender: "male",
      bannerMale: [],
      bannerFemale: [],
      maleHot: null,
      maleFinish: null,
      maleLiuchunlv: null,
      femaleHot: null,
      femaleFinish: null,
      femaleLiuchunlv: null,
    };
  },
  created() {
    this.axios.get("/banner.json").then((response) => {
      this.bannerMale = response.data.male;
      this.bannerFemale = response.data.female;
    });

    this.axios
      .get("api/ranking/54d42d92321052167dfb75e3")
      .then((response) => {
        // console.log(response);
        this.maleHot = response.data.ranking;
        // console.log(this.maleHot);
      });

    this.axios
      .get("api/ranking/564eb878efe5b8e745508fde")
      .then((response) => {
        this.maleFinish = response.data.ranking;
      });

    this.axios
      .get("api/ranking/564547c694f1c6a144ec979b")
      .then((response) => {
        // console.log(response);
        this.maleLiuchunlv = response.data.ranking;
        // console.log(this.maleLiuchunlv);
      });

      // 女生
      this.axios
      .get("api/ranking/54d43437d47d13ff21cad58b")
      .then((response) => {
        // console.log(response);
        this.femaleHot = response.data.ranking;
        // console.log(this.maleLiuchunlv);
      });

      this.axios
      .get("api/ranking/564eb8a9cf77e9b25056162d")
      .then((response) => {
        // console.log(response);
        this.femaleFinish = response.data.ranking;
        // console.log(this.maleLiuchunlv);
      });

      this.axios
      .get("api/ranking/5645482405b052fe70aeb1b5")
      .then((response) => {
        // console.log(response);
        this.femaleLiuchunlv = response.data.ranking;
        // console.log(this.maleLiuchunlv);
      });
  },
  beforeRouteEnter(to, from, next) {
    // 在渲染该组件的对应路由被 confirm 前调用
    // 不！能！获取组件实例 `this`
    // 因为当守卫执行前，组件实例还没被创建
    next((vm) => {
      // 通过 `vm` 访问组件实例
      vm.gender = to.query.gender || "male";
    });
  },
  beforeRouteUpdate(to, from, next) {
    this.gender = to.query.gender || "male";
    next();
  },
};
</script>

<style scoped lang="less">
.home {
  // background-color: white;
  .van-tabbar {
    // height: 111.3px;
    padding: 13px 10px;
    box-sizing: border-box;
  }
  .zuijin {
    width: 100%;
    height: 34px;
    line-height: 34px;
    padding: 0 18px;
    box-sizing: border-box;
    text-align: left;
    font-size: 12px;
    color: #999;
    // margin-top: 13px;
    background-color: white;
    span {
      margin-right: 8px;
    }
    span:last-child {
      color: red;
    }
  }
}
</style>
